var payqrButton = {
    ...
    init: function(merchant_id, token){
        ...
        this.token = token;
        this.button = this.clearUserData();
        this.createButton();
    },
    ...
    getTotal: function (){
        var total = 0;
        try {
            var cart= $.parseJSON(sessionStorage.getItem('cookieCart'));
            if (cart)
            {
...
$(function(){
    ...
    var merchant_id = getMerchantId();

    function getMerchantId()
    {
        var merchant_id;
        $("script[src]").each(function() {
            if($(this).attr("src").match(/merchId/i) != null)
            {
                var srcMerchId = $(this).attr("src").split("merchId=");

                if(srcMerchId[1] != "undefined")
                {
                    merchant_id = srcMerchId[1];
                }
            }
        });
    ...
    if(typeof payQR  !== "undefined"){
        ...
        payQR.onPaid(function(data) {
            ...
            try{
                ...
                //здесь производим очистку корзины, если удаление производится через корзину товаров
                ...
                //очищаем товары самостоятельно формируя ссылку
                ...
                if (cart)
                {
                    $.each(cart.order_lines, function(index, order_line){
                        $.ajax({
                            type: 'post',
                            url: /cart_items/ + order_line.variant_id,
                            data: {_method:'delete'}
                        });
                    })
                }

                sessionStorage.removeItem('cookieCart');
    ...
    $('#order_form, #contacts, #delivery, #payment').on('change', function(){
        if(this.id == "contacts" || event.target.id == "client_name" || event.target.id == "client_phone" || event.target.id == "client_email")
        {
            window.sessionStorage.setItem('button', JSON.stringify(payqrButton.clearUserData()));
        }
        payqrButton.init(merchant_id, insales);
    ...
    /**
     * Сохраняем в хранилице информацию о корзине
     */
    $('input[type="submit"][class*="cart-button"]').click(function(){
        if($.cookie('cart'))
        {
            sessionStorage.setItem('cookieCart', $.cookie('cart'));
